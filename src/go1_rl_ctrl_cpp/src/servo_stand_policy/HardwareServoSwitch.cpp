...
bool HardwareServoSwitch::UDPRecv()
{
  udp.Recv();
  udp.GetRecv(state);
  return true;
...
void HardwareServoSwitch::paramInit()
{
  for(int i=0; i<2; i++){
...
bool HardwareServoSwitch::send_cmd() {
  double pos[12] = {0.0, 0.67, -2.0, -0.0, 0.67, -2.0,
...
void HardwareServoSwitch::moveAllPosition(double* targetPos, double duration) {
