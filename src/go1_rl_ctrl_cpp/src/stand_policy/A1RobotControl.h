...
public:
    ...
    void compute_joint_torques(Go1CtrlStates &state);

    Eigen::Matrix<double, 3, NUM_LEG> compute_grf(Go1CtrlStates &state, double dt);

    Eigen::Vector3d compute_walking_surface(Go1CtrlStates &state);
